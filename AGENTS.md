...
- **Licenses:** third-party code may use MIT, BSD, Apache-2.0, APSL-2.0, or public-domain terms, plus Apple frameworks. **Never** add GPL / LGPL / AGPL. APSL-2.0 is allowed only when its compliance requirements below are met; it does not relicense this MIT project or unrelated code. If a task seems to require a license outside this allowlist, **STOP**, record a blocker in `current_status.md`, and move to an unblocked slice. The CI SPDX guard must continue to reject copyleft text.
- **APSL-2.0 compliance:** retain every upstream copyright, proprietary notice, disclaimer, and APSL reference; keep the complete APSL-2.0 text with distributed or externally deployed Covered Code source and documentation; and do not impose terms that alter or restrict APSL rights. For every modification, add the APSL Exhibit A notice to each modified source file and prominently record that it was changed and when. If modified Covered Code is externally deployed, make the source of those modifications available to recipients or publicly under APSL-2.0 for the longer of the deployment period or 12 months. Executable-only external deployments must prominently explain that source is available under APSL-2.0 and how to obtain it. Do not use Apple trademarks or imply Apple endorsement. See the [official APSL-2.0 text](https://opensource.org/license/APSL-2.0).
...
- **Warnings are build failures:** keep first-party Swift, C, C++, C#, and shell validation warning-free where the toolchain supports warnings-as-errors. Do not hide a warning with a broad suppression; for vendored code, use a narrow system-include or compiler-boundary treatment while preserving the source unmodified, and record any unavoidable toolchain warning in `current_status.md`.

...
The native C sources and public/internal headers are unmodified. The shared libogg core sources
(`bitwise.c` and `framing.c`) are built by the sibling `Cogg` target and are excluded from this
target to keep one Ogg implementation in the native build. The optional platform-specific

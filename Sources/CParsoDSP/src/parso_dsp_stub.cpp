...
struct pd_conv {
    ...
    int parts = 0;
    float irGain = 1.0f;
    std::vector<std::complex<float>> irSpec;   // parts * NB
    ...
    void runBlock() {
        ...
        fft.fft(inWin.data(), reinterpret_cast<std::complex<float>*>(xSpec.data()));
        // Push current spectrum into the frequency-domain delay line.
        ...
        for (int p = 0; p < parts; ++p) {
            ...
            const std::complex<float>* hs = &irSpec[static_cast<size_t>(p) * NB];
            for (int k = 0; k < NB; ++k) ySpec[k] += xs[k] * hs[k];
        }
        fdlHead = (fdlHead + 1) % MAX_PARTS;
        fft.ifft(reinterpret_cast<const std::complex<float>*>(ySpec.data()), timeOut.data());
        const float norm = irGain / static_cast<float>(N);
        for (int i = 0; i < B; ++i) {
            float v = timeOut[B + i] * norm;
...
int pd_conv_set_ir(pd_conv* c, const float* ir, int ir_len) {
    ...
    for (int p = 0; p < parts; ++p) {
        ...
        c->fft.fft(block.data(), reinterpret_cast<std::complex<float>*>(spec.data()));
        std::complex<float>* dst = &c->irSpec[static_cast<size_t>(p) * NB];
        for (int k = 0; k < NB; ++k) dst[k] = spec[k];
    }
    // Reset run state.
    std::fill(c->fdl.begin(), c->fdl.end(), std::complex<float>{});
    c->fdlHead = 0;
    c->inWin.fill(0.0f);
    c->inAccum.fill(0.0f);
    c->inFill = 0;
    c->outBlock.fill(0.0f);
    c->outPos = B;
    c->ready = true;
    return PD_OK;
}

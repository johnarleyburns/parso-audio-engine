...
extension CachingResourceLoader {
    ...
    public func warm(upTo bytes: Int64) {
        ...
        let url = originalURL
        let cfg = config
        let cacheStore = self.store
        let requestSession = session
        Task.detached(priority: .background) { [key, url, cfg, cacheStore, requestSession, bytes] in
            guard bytes > 0 else { return }
            let fileURL = await cacheStore.fileURL(for: key)
            if !FileManager.default.fileExists(atPath: fileURL.path) {
                FileManager.default.createFile(atPath: fileURL.path, contents: nil)
            }
            let already = await cacheStore.rangeMap(for: key).contiguousBytes(from: 0)
            ...
            do {
                let (body, response) = try await requestSession.data(for: req)
                ...
                case .fullBody(let total):
                    if already > 0 { return }
                    await cacheStore.setContentLength(total, for: key)
                    writeOffset = 0
                }
                ...
                await cacheStore.recordWrite(range: writeOffset..<(writeOffset + Int64(body.count)), for: key)
            } catch {}
        }
    }
}

...
4. **Decode scope:** FLAC (libFLAC/`Cflac`), **Ogg Vorbis** (libvorbisfile/`Cvorbis`), **Opus** (libopus+libopusfile/`Copus`), plus MP3/AAC/ALAC/WAV/AIFF/CAF and audiobook M4B (AAC/ALAC in ISO-BMFF) through the validated platform adapter or portable demux/codec path.
5. **Encode scope:** WAV/PCM, FLAC (libFLAC), AAC and ALAC (AVFoundation or a validated portable path), audiobook M4B (AAC in ISO-BMFF), and MP3 via the
...
| `Cogg` + `Cvorbis` | libogg + libvorbis/libvorbisenc/libvorbisfile | BSD-style | Ogg Vorbis encode + decode |
...
size are too widely spaced to represent semitones. The track estimator uses square-root magnitude
compression over the full configured frequency range and no harmonic reinforcement; do not
independently normalize or boost a bass-only profile, which misidentifies inverted chords whose
root and third fall above its cutoff. The public chroma helper can additionally reinforce
fundamentals at ÷2/÷3/÷4, weighted by 1/harmonic. Blend the normalized frame mean (40%) and
per-pitch median (60%).

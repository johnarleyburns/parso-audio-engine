...
| Device playback / capture | Apple audio frameworks | Host callback plus PipeWire `pw-cat` adapter supported; route restart/hot-unplug acceptance pending | WASAPI adapter/routes pending | Device facade pending | Oboe adapter/routes pending | `parso_linux_pipewire_host` live playback/capture and CTest no-device contract | CP4/CP5 / CP-WIN |

...
The Swift library has been confirmed by the maintainer in two consuming applications, and the Linux
native path has been confirmed on the maintainer's desktop. Windows hardware acceptance and Android
device/route validation are unavailable because the required hardware is not available; CI build,
consumer, and packaging evidence is retained, but neither platform is claimed as device-validated in
this release.

| Surface | Swift / Apple | C/C++ / Linux | Windows C/C++ | Windows C# | Kotlin / Android | CP0 evidence | First implementation phase |
|---|---|---|---|---|---|---|---|
| Real-time DSP and render graph | Supported through `CParsoDSP` / `CParsoEngine` | Supported through CMake/CTest plus dependency-free host callback; device backend pending | MSVC build/runtime and consumers pass CI; Windows hardware acceptance unavailable | Managed wrapper and native consumer pass CI; Windows hardware acceptance unavailable | Same native library builds through NDK/JNI; Android device unavailable | CMake 18-test suite, allocator-instrumented variable callback test, sanitizer subset, host callback, and ABI export checks | CP1 / CP-WIN |
| Headless `pe_step` rendering | Existing test path | Supported public facade and CTest | Native CTest/runtime pass CI; Windows hardware acceptance unavailable | Managed render, retained deck PCM, play/pause, record, crossfader, and synchronized lifetime surface built; native CI consumer passes | JNI direct-buffer render, mixer commands, copied stats/events, and record tap are packaged for arm64-v8a/x86_64; device execution unavailable and intentionally outside CI | `pe_step` is the common deterministic seam | CP2 / CP4 / CP-WIN |
| DJ controls and state | Swift `ParsoDJEngine` | Partial shared command ABI: transport, cues, loops, hot cues, slip/keylock, crossfader; full parity pending | Native CTest/consumer runtime passes CI; full parity and hardware acceptance unavailable | Matching managed command/control surface and native CI consumer pass; full parity and hardware acceptance unavailable | Kotlin exposes mixer control plus typed cue/hot-cue/loop/seek/sync/key-lock/slip commands; JNI lifecycle/render/control is compiled and packaged; device execution unavailable and intentionally outside CI | C11/C++17/Python/C#/Kotlin command consumers cover implemented selectors; Android device instrumentation remains a manual gate | CP3 / CP-WIN |
...
| FLAC / Vorbis / Opus | Supported through vendored C | Xiph Vorbis plus native FLAC/Opus bridges are fixture-gated and pass | Cross-build/capability evidence; native MSVC runtime pending | Managed capability and byte-codec wrapper built; native Windows run pending | `ParsoVorbis` is packaged in the AAR for both ABIs; physical codec/runtime fixture gate pending and intentionally outside CI | Permissive vendor records, real fixture consumers, ABI capability checks, and Android device testing | CP1/CP3 / CP4 / CP-WIN |
| WAV / PCM | Supported | Supported through `parso_wav_*` / `parso_pcm_*` and installed consumers | Native C/C++ runtime passes CI; hardware acceptance unavailable | C# buffer/marshaling surface and native consumer pass CI; hardware acceptance unavailable | Direct-buffer JNI input/output seam and published AAR pass static/package gates; physical device unavailable and intentionally outside CI | C11/C++17 CTest, installed package consumer, AAR, and external consumer instrumentation compilation | CP3 / CP-WIN |
| SRC / loudness | Supported through `Csrc` / `Cebur128` | Supported through `parso_src_convert` / `parso_loudness_measure` | Native runtime passes CI; hardware acceptance unavailable | `CodecServices` wraps copied SRC and EBU R128 results; native CI consumer passes; hardware acceptance unavailable | `ParsoOffline` wraps copied SRC and EBU R128 results; producer instrumentation is compiled and packaged; physical device unavailable and intentionally outside CI | C11/C++17/Python/C# service consumers, installed package consumers, real fixture measurements, and Android device testing | CP3 / CP4 / CP-WIN |
...
| Recording | Swift `MixRecorder` | C ABI record tap plus C++17 `MixRecorder` and Python control-side WAV/FLAC/AAC encoders | Public bounded record tap and native acceptance drain; full file service partial | `MixRecorder` encodes drained stereo blocks to WAV/FLAC/AAC; native DLL execution pending | `ParsoEngine` activation/drain/drop/reset and `ParsoRecorder` WAV encoding are packaged in the AAR; device runtime pending and intentionally outside CI | Record-ring overflow/long-session, C++/C#/Python round trips, and Android instrumentation compilation | CP3 / CP4 / CP-WIN |
| Device playback / capture | Apple audio frameworks | Host callback plus PipeWire `pw-cat` adapter supported; bounded process restart and named-route recording recovery covered; physical hot-unplug/latency acceptance pending | Event-driven shared-mode WASAPI render/capture host cross-builds; route recovery, native endpoint execution, and latency pending | Device facade pending | Oboe output/capture adapter plus Kotlin focus/route lifecycle implemented; physical route/latency acceptance pending | `parso_linux_pipewire_host` live playback/capture, no-device contract, deterministic recovery CTest, and 30-second named-route restart runner; Android Oboe callback lifecycle instrumentation passes; Windows PE host build passes | CP4/CP5 / CP-WIN |
| Human listening acceptance | Confirmed by maintainer in two consuming apps | Native/Python 30-second real-MP3-deck WAV/JSON artifacts, index, runner, parity report, and 30-second named-route recording bundle built; maintainer accepted the available desktop artifacts | CI render/consumer checks pass; Windows listening unavailable due hardware | C# artifacts not runtime-verified | Device review unavailable due hardware | Existing WAV + JSON + MP4 contract reused | CP6 / CP-WIN |
| Packaging | SwiftPM / Xcode | CMake package, pkg-config metadata, installed C11/C++17 consumers, and ABI reports pass | MinGW x86_64 cross-build emits `libparso.dll`, public consumers, and `parso_windows_wasapi_host.exe`; native MSVC package/runtime pending | Windows-targeted .NET assembly builds on Linux; native DLL/NuGet runtime pending | Release AAR with Oboe, local Maven publication, external consumer APK, ABI/JNI checks, and instrumentation compilation pass with pinned NDK/CMake; physical device gate pending and intentionally outside CI | External Linux C11/C++17 consumers, Android AAR/APK contents, JNI exports, all shipped 16 KiB ELF libraries, and instrumentation compilation | CP2/CP4/CP5 / CP-WIN |

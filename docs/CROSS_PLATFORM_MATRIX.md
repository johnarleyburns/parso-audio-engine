...
| Headless `pe_step` rendering | Existing test path | Public facade and CTest in progress | Native CTest on `windows-latest` | Managed render, retained deck PCM, play/pause, and record consumer added; native run pending | Planned JNI smoke path | `pe_step` is the common deterministic seam | CP2 / CP-WIN |

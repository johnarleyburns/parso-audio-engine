...
| Headless `pe_step` rendering | Existing test path | Public facade and CTest in progress | Native CTest on `windows-latest` | Managed render, retained deck PCM, play/pause, and record consumer added; native run pending | JNI direct-buffer create/destroy/play/render seam builds for arm64-v8a/x86_64; device pending | `pe_step` is the common deterministic seam | CP2 / CP4 / CP-WIN |
...
| Analysis | Swift + Accelerate | Native summary analysis (duration/levels/tempo) supported; key/structure planned | Shared C ABI and C++ wrapper summary path; native Windows execution pending | C# summary result and call surface added; native Windows execution pending | Planned through native analysis API | Synthetic 120 BPM summary gate passes in C11/Python; full algorithms remain pending | CP3 / CP-WIN |
| FLAC / Vorbis / Opus | Supported through vendored C | Xiph Vorbis plus native FLAC/Opus bridges are fixture-gated | Planned native MSVC and cross-build capability checks | Managed capability and byte-codec wrapper added; native Windows run pending | Planned after NDK fixture gates | Vendor licenses and targets inventoried | CP1/CP3 / CP-WIN |
| WAV / PCM | Supported | Supported through `parso_wav_*` / `parso_pcm_*` | Native C/C++ offline consumers pass with the shared ABI | C# buffer/marshaling surface is present; native consumer pending | Planned | CP3 native capability + ownership slice is covered by independent C11/C++17 CTest consumers; packaging remains pending | CP3 / CP-WIN |
...
| Recording | Swift `MixRecorder` | Public record tap plus native off-thread service planned | Planned native recorder and Windows consumer tests | Managed record-tap methods added; native consumer run pending | Planned native service with Kotlin lifecycle API | Existing record ring is the native seam | CP3 / CP-WIN |

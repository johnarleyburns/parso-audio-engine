...
| Device playback / capture | Apple audio frameworks | Host callback plus PipeWire `pw-cat` adapter supported; bounded process restart and named-route recording recovery covered; physical hot-unplug/latency acceptance pending | Event-driven shared-mode WASAPI render host cross-builds; capture, route recovery, and native endpoint execution pending | Device facade pending | Oboe output/capture adapter plus Kotlin focus/route lifecycle implemented; physical route/latency acceptance pending | `parso_linux_pipewire_host` live playback/capture, no-device contract, deterministic recovery CTest, and 30-second named-route restart runner; Android Oboe callback lifecycle instrumentation passes; Windows PE host build passes | CP4/CP5 / CP-WIN |
